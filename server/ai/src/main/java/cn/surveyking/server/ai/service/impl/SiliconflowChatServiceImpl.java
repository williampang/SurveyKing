...
/**
 * SiliconFlow 聊天服务实现类 支持通过SiliconFlow平台调用多种AI模型，包括DeepSeek、Qwen、Llama等
...
public class SiliconflowChatServiceImpl implements AiChatService {
	...
	public List<ModelType> getSupportedModels() {
		...
		return aiSetting.getModels().stream().map(model -> new ModelType(model, model, "AI模型: " + model))
	...
	public Flux<StreamResponseEvent> createChatStream(ChatRequest chatRequest, String conversationId, String model,
		...
		return webClient.post().uri(SILICONFLOW_BASE_URL + "/v1/chat/completions")
				.header("Authorization", "Bearer " + aiSetting.getToken()).header("Content-Type", "application/json")
				.bodyValue(requestBody).retrieve().bodyToFlux(String.class).timeout(Duration.ofMinutes(5))
				.mapNotNull(original -> {
